...
export default function Decor({}) {
  ...
  const getLastIdForCategory = () => {
    if (data.category) {
      const params = {
        getLastIdFor: data.category,
      };
      const queryString = new URLSearchParams(params).toString();
      setLoading(true);
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/decor?${queryString}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const isIdValid = () => {
    return new Promise((resolve, reject) => {
      if (!data.productInfo.id) {
        resolve(false);
      } else {
        fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/decor?checkId=${data.productInfo.id}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const idValidity = await isIdValid();
    if (!idValidity) {
      alert("Decor Id is used before enter another Id.");
      setLoading(false);
    } else if (!data.name) {
      alert("Enter Name");
      setLoading(false);
      ...
      setLoading(false);
    } else if (!data.imageFile || !data.thumbnailFile) {
      alert("Upload Image/Thumbnail");
      setLoading(false);
    } else {
      try {
        let image = "";
        ...
        let tempproductVariantsImageFiles = data.productVariants.map(
          (i) => i.imageFile
        );
        ...
        if (tempproductVariantsImageFiles.length > 0) {
          tempproductVariantsImages = await Promise.all(
            tempproductVariantsImageFiles.map(
              async (i, index) =>
                await uploadFile({
                  file: i,
        ...
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/decor`, {
          ...
          .then((response) => {
            if (response.message !== "error") {
              ...
              router.push(`/wedding-store/view?id=${response.id}`);
            } else {
  ...
  useEffect(() => {
    if (data.category) {
      getLastIdForCategory();
      let tempCategory = categoryList.find((i) => i.name === data.category);
      let tempAttributes = tempCategory.attributes.map((i) => ({
        name: i,
        list: [],
      }));
      let tempProductTypes = tempCategory.productTypes.map((i) => ({
        name: i,
        costPrice: 0,
        sellingPrice: 0,
        discount: 0,
      }));
      tempProductTypes = tempProductTypes.sort((a, b) =>
  ...
  return (
    <div className="p-8 flex flex-col gap-6 relative">
      ...
      <div className="flex flex-col gap-4">
        <p className="font-semibold text-2xl">Add New Decor</p>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (
                                ...
                                setData({
                                  ...data,
                                  productAddOns: [...data.productAddOns, item],
                                });
                                setAddProductAddOns({
                                  ...addProductAddOns,
                                  display: false,
