...
export default function Decor({ user }) {
  ...
  const fetchList = () => {
    setLoading(true);
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/decor-package?page=${
        page || 1
  ...
  const deleteDecorPackage = (id) => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/decor-package/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      .then((response) => {
        if (response.message === "success") {
          ...
          alert("Error deleting Decor Package, try again");
        }
      })
      .catch((error) => {
        console.error("There was a problem with the fetch operation:", error);
        ...
        <div className="bg-white rounded-3xl flex-col flex gap-3 shadow-xl">
          <div className="flex flex-row justify-between items-center pt-4 px-12">
            <div className="flex flex-col">
              <p className="text-lg font-medium">Decor Packages</p>
            </div>
            <div className="flex flex-row gap-4">
              <Link href={"/wedding-store/create-package"}>
                  ...
                  >
                    ...
                    <Table.Cell className="flex gap-2">
                      ...
                      <MdDelete
                        ...
                        onClick={() => {
                          if (
                            confirm("Do you want to delete the Decor Package?")
                          ) {
                            deleteDecorPackage(item._id);
                          }
                        }}
                      />
                    </Table.Cell>
