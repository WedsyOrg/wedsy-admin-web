...
export default function Decor({ user }) {
  ...
  const fetchList = () => {
    setLoading(true);
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/decor?label=popular&page=${
        page || 1
  ...
  const fetchDecor = () => {
    setLoading(true);
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/decor?searchFor=decorId&decorId=${popularSearch}&limit=5`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const addPopularDecor = () => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/decor/${popular}?addTo=popular`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      .then((response) => {
        if (response.message === "success") {
          alert("Decor Added to popular Successfully");
          fetchList();
          onCloseModal();
        } else {
  ...
  const deletePopularDecor = (id) => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/decor/${id}?removeFrom=popular`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      .then((response) => {
        if (response.message === "success") {
          alert("Decor Removed from popular Successfully");
          fetchList();
          onCloseModal();
        } else {
                ...
                <div className="flex flex-row flex-wrap gap-2">
                  {popularList.map((item, index) => (
                    <Button
                      key={index}
                      color="gray"
                      onClick={() => setPopular(item._id)}
                      disabled={loading || popular === item._id}
                    >
        ...
        <div className="bg-white rounded-3xl flex-col flex gap-3 shadow-xl">
          <div className="flex flex-row justify-between items-center pt-4 px-12">
            <div className="flex flex-col">
              <p className="text-lg font-medium">Popular Decor</p>
            </div>
                    ...
                    <Table.Cell className="flex gap-2">
                      ...
                      <MdDelete
                        ...
                        onClick={() => {
                          if (
                            confirm(
                              "Do you want to remove the Decor from Popular?"
                            )
                          ) {
                            deletePopularDecor(item._id);
                          }
                        }}
                      />
                    </Table.Cell>
